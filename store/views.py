...
def index(request):
    ...
    jaykishan = {'allProds': allProds}
    return render(request, 'store/index.html', jaykishan)
...
def orderView(request):
    if request.user.is_authenticated:
        ...
        if len(orderHistory) == 0:
            ...
            return render(request, 'store/orderView.html')
        return render(request, 'store/orderView.html', {'orderHistory': orderHistory})
    return render(request, 'store/orderView.html')
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    jaykishan = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        jaykishan = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'store/search.html', jaykishan)
...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        order.save()
        update = OrderUpdate(order_id=order.order_id, update_desc="Pending")
        update.save()
        thank = True
        id = order.order_id
        if 'onlinePay' in request.POST:
            ...
            jaykishan_dict = {
                ...
                'MID': 'kbzk1DSbJiV_O3p5',  # Your-Merchant-Id-Here
            ...
            jaykishan_dict['CHECKSUMHASH'] = Checksum.generate_checksum(jaykishan_dict, MERCHANT_KEY)
            return render(request, 'store/paytm.html', {'jaykishan_dict': jaykishan_dict})
